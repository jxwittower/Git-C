...
for r in tail(ci,0)+tail(ci,5)+tail(co,0)+tail(co,6):
 require(r['main']==0,'FINAL_EMPTY_STATE_FAIL');require(r['hr']==0 and r['rr']==0,'FINAL_EMPTY_VITAL_FAIL')
warn=[r for r in co if r['source']==4 and r['main']==8];require(len(warn)>0,'DEPARTURE_WARNING_STATE8_MISSING');require(all(r['hr']==0 and r['rr']==0 for r in warn),'WARNING_VITAL_SUPPRESSION_FAIL')
ret=[r for r in co if r['source']==5];require(any(r['main'] not in (0,8) for r in ret),'RETURN_OCCUPANCY_RECOVERY_FAIL');require(any((r['hr']>0 or r['rr']>0) for r in ret[len(ret)//2:]),'RETURN_VITAL_RECOVERY_FAIL')
occ=[r for r in ci if r['source'] in (1,2,3,4)];require(any(r['main'] not in (0,8) for r in occ),'INNER_OCCUPIED_STATE_MISSING');require(any(r['hr']>0 for r in occ) and any(r['rr']>0 for r in occ),'INNER_VITALS_MISSING');print('SEQ_TARGETS_VITAL_SUPPRESSION_RECOVERY_PASS')
...
def vital_compare(a,b,name):
 ...
 for r in a:
  ...
  if q and r['main'] not in (0,8) and q['main'] not in (0,8):
   if r['hr']>0 and q['hr']>0:dh.append(abs(r['hr']-q['hr']))
   if r['rr']>0 and q['rr']>0:dr.append(abs(r['rr']-q['rr']))
...
print('PERFORMANCE_AND_STABILITY_PASS');print('COMPLETE_NATIVE_WINDOWS_REGRESSION_PASS')
