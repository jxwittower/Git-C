...
if(-not (Select-String candidate_headers.txt -SimpleMatch '14C machine (x86)' -Quiet)){throw 'PE32_X86_FAIL'}
if(-not (Select-String candidate_headers.txt -SimpleMatch '.stkfix' -Quiet)){throw 'STKFIX_SECTION_FAIL'}
foreach($e in @('_SigProc@24','_SigProc_Fini@0','_SigProc_Init@8','_SigProc_Stat@0')){if(-not (Select-String candidate_exports.txt -SimpleMatch $e -Quiet)){throw "EXPORT_FAIL $e"}}
@'
...
if(-not (Select-String loader_inner.log -SimpleMatch 'ALL_LOADER_CONTRACT_TESTS_PASS' -Quiet)){throw 'INNER_LOADER_MARKER_FAIL'}
if(-not (Select-String loader_outer.log -SimpleMatch 'ALL_LOADER_CONTRACT_TESTS_PASS' -Quiet)){throw 'OUTER_LOADER_MARKER_FAIL'}
...
foreach($x in $markers){if(-not (Select-String $x.f -SimpleMatch $x.m -Quiet)){throw "MARKER_FAIL $($x.m)"}}
