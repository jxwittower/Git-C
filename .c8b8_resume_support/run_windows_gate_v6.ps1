...
# Exact original HostCompat is retained only as static ABI/import reference. Dynamic execution is forbidden: it has 12 unprobed large stack reservations and has produced 0xC0000005 in real Windows nonzero and loader/init paths.
'ORIGINAL_HOSTCOMPAT_DYNAMIC_SKIPPED_KNOWN_STACK_UNSAFE SHA256=081c4c62d2bf377913eb2aca0cc3cf3bf656350c8abaa3aaf11d171dac604804 LARGE_DIRECT_STACK=12 WINDOWS_AV=0xC0000005' | Set-Content hostcompat_original_runtime_defect.txt

# Deterministic ba15 HostCompat stack-probe repair must preserve the loader contract in both runtime layouts.
$rtHFI=Copy-Runtime 'payload\runtime\inner-bed' 'rt_hostfix_loader_inner'
$rtHFO=Copy-Runtime 'payload\runtime\outer-medical-bed' 'rt_hostfix_loader_outer'
foreach($rt in @($rtHFI,$rtHFO)){Copy-Item $hostFixed "$rt\SigProcDll-64HF.dll" -Force;Copy-Item $smoke "$rt\SigProcLoaderContractSmoke.exe" -Force}
Push-Location $rtHFI; .\SigProcLoaderContractSmoke.exe *> "$root\hostfix_loader_inner.log"; $rc=$LASTEXITCODE; Pop-Location; Get-Content hostfix_loader_inner.log; if($rc -ne 0){throw "HOSTFIX_INNER_LOADER_FAIL $rc"}
Push-Location $rtHFO; .\SigProcLoaderContractSmoke.exe *> "$root\hostfix_loader_outer.log"; $rc=$LASTEXITCODE; Pop-Location; Get-Content hostfix_loader_outer.log; if($rc -ne 0){throw "HOSTFIX_OUTER_LOADER_FAIL $rc"}
if(-not (Select-String -Path hostfix_loader_inner.log -Pattern 'ALL_LOADER_CONTRACT_TESTS_PASS' -SimpleMatch -Quiet)){throw 'HOSTFIX_INNER_LOADER_MARKER_FAIL'}
if(-not (Select-String -Path hostfix_loader_outer.log -Pattern 'ALL_LOADER_CONTRACT_TESTS_PASS' -SimpleMatch -Quiet)){throw 'HOSTFIX_OUTER_LOADER_MARKER_FAIL'}
