...
def state(page,name):
 ...
 # lightweight UI inventory
 d['inputs']=[]
 for i in range(min(page.locator('input,textarea').count(),60)):
  e=page.locator('input,textarea').nth(i)
  d['inputs'].append({'i':i,'placeholder':e.get_attribute('placeholder') or '', 'value':e.input_value() if e.is_editable() else (e.get_attribute('value') or ''), 'type':e.get_attribute('type') or '', 'class':e.get_attribute('class') or ''})
 d['buttons']=[]
...
def load_home(page,channel):
 page.goto(BASE,wait_until='domcontentloaded',timeout=35000); page.wait_for_timeout(1500)
 if channel!='多领域':
...
def run_query(page,channel,q,idx):
 ...
 try:
  ...
  # screenshot conditions before submit for audit trail
...
def click_feature(page,label,name):
 # click first visible control whose text contains label
...
def feature_probe(page):
 ...
 try:
  load_home(page,'新闻')
  page.locator('input[placeholder*="BCC检索式"]').first.fill('人工智能')
  page.locator('button.search-btn').first.click(); page.wait_for_timeout(3500)
  shot(page,'90_feature_base_result',True)
  for label in ['整句','统计','实例统计','二次筛选','筛选','历时','下载','出处']:
   # reload result before each attempt to keep independent
   load_home(page,'新闻'); page.locator('input[placeholder*="BCC检索式"]').first.fill('人工智能'); page.locator('button.search-btn').first.click(); page.wait_for_timeout(2500)
...
def capture_static(page):
 pages=[
  ('00_home',BASE),('80_help',BASE+'help.html'),('81_download',BASE+'download'),('82_langsc',BASE+'LangSC.html'),('83_personal_bcc',BASE+'build-corpus.html')]
 out=[]
 for name,u in pages:
  try:
   page.goto(u,wait_until='domcontentloaded',timeout=35000); page.wait_for_timeout(2200)
...
def main():
 ...
 with sync_playwright() as p:
  ...
  page=ctx.new_page()
  log['static']=capture_static(page)
  idx=1
  for channel,qs in TESTS.items():
   for q in qs:
    log['queries'].append(run_query(page,channel,q,idx)); idx+=1
  log['features']=feature_probe(page)
  browser.close()
